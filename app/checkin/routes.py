...
def update_checkin():
    ...
    try:
        ...
        for guest_data in data['Guests']:
            ...
            guest = Guest.query.filter_by(DocumentNumber=guest_data['DocumentNumber']).first()
            if not guest:
                # This handler only ever amends guests that are already on the
                # stay. An unknown document number used to fall straight through
                # to `guest.FirstName` on None and escape as a 500.
                return jsonify({
                    "hasErrors": True,
                    "errorMessages": {"general": "50018 Guest is not on this check-in"},
                    "messageType": "CheckinResponse",
                    "clientUID": establishment_uid,
                    "messageUID": str(uuid.uuid4()),
                    "correlationUID": data.get('CorrelationUID', str(uuid.uuid4())),
